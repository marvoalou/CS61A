...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        while expr is not nil:
            return scheme_apply(procedure, expr.rest.map(lambda x: scheme_eval(x, env)), env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        #特殊处理scheme_eval(Pair(1, nil))的情况
        value = scheme_eval(expressions.rest.first, env) if expressions.rest.rest is nil else scheme_eval(expressions.rest, env)
