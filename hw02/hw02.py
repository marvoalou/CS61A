...
def product(n, term):
    ...
    def reduce(f,item):
        ...
        for x in item:
            mul*=f(x)
        return mul
    return reduce(term,range(1,n+1))
...
def make_repeater(func, n): #p
    ...
    625
    >>> make_repeater(square, 4)(5) # square(square(square(square(5))))     f(make(3)(5))
    152587890625
    >>> make_repeater(square, 0)(5) # Yes, it makes sense to apply the function zero times!
    5
    """
    "*** YOUR CODE HERE ***"
    #lambda x: func  使其返回func,由于没有用到x，所以只返回函数本身
